...
<div class="container-fluid">
	<form action="" id="field-form">
		...
		<div class="form-group">
			<label for="name" class="control-label">Naziv polja</label>
			<input type="text" name="name" id="name" class="form-control form-control-sm rounded-0" value="<?php echo isset($name) ? $name : ''; ?>"  required/>
		</div>
		<div class="form-group">
			<label for="description" class="control-label">Kratak opis</label>
		...
		<div class="form-group">
			...
			<option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Aktivan</option>
			<option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Neaktivan</option>
			</select>
		</div>
	</form>
</div>
